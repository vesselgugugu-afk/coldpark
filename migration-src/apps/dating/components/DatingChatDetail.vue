...
watch(() => props.show, async (val) => {
  if (val && props.chatId) {
    chatData.value = bridge.hasSdk()
      ? await bridge.dbGet('matches', props.chatId)
      : await db.dating_chats.get(props.chatId)
    if (chatData.value) {
      chatProfile.value = bridge.hasSdk()
...
const pushLocalMessage = async (msgObj, targetChatId = props.chatId) => {
  ...
  if (bridge.hasSdk()) {
    await bridge.writeHistory({
      sessionId: chatData.value?.sessionId,
      characterId: chatData.value?.characterId || chatProfile.value?.realCharId,
      role: fullMsg.role,
      type: fullMsg.type,
  ...
  const cData = bridge.hasSdk() ? await bridge.dbGet('matches', targetChatId) : await db.dating_chats.get(targetChatId)
  if (cData && fullMsg.role === 'user') {
    if (bridge.hasSdk()) await bridge.dbUpdate('matches', targetChatId, { messageCount: (cData.messageCount || 0) + 1 })
    else await db.dating_chats.update(targetChatId, { messageCount: (cData.messageCount || 0) + 1 })
  }
}
