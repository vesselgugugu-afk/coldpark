...
const openPendingDm = async (msg) => {
  ...
  try {
    ...
    if (msg.pendingRealCharId) {
      ...
      const profile = bridge.hasSdk()
        ? await bridge.dbCreate('profiles', fakeProfile)
        : { id: await db.dating_profiles.add(fakeProfile) }
      const match = bridge.hasSdk()
        ? await bridge.dbCreate('matches', {
            profileId: profile.id,
            characterId: msg.pendingRealCharId,
            messageCount: 0,
            status: 'active',
            type: 'feed'
          })
        : { id: await db.dating_chats.add({
        profileId: profile.id,
        messageCount: 0,
        status: 'active',
        type: 'feed'
