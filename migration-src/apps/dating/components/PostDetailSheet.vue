  ...
  try {
    ...
    if (bridge.hasSdk()) {
      await bridge.writeHistory({
        sessionId: (await bridge.dbGet('matches', chatId))?.sessionId,
        characterId: (await bridge.dbGet('matches', chatId))?.characterId,
        role: 'user',
        type: fullMsg.type,
    ...
    const targetChat = bridge.hasSdk() ? await bridge.dbGet('matches', chatId) : await db.dating_chats.get(chatId)
    if (targetChat) {
      const data = { messageCount: (targetChat.messageCount || 0) + 1 }
      if (bridge.hasSdk()) await bridge.dbUpdate('matches', chatId, data)
      else await db.dating_chats.update(chatId, data)
    }
