...
const loadChats = async () => {
  try {
    if (bridge.hasSdk()) {
      const [chats, profiles, history] = await Promise.all([
        bridge.dbList('matches'),
        bridge.dbList('profiles'),
        bridge.dbList('chat_history')
      ])
      const lastMsgMap = {}
      for (const msg of history) {
        const key = msg.sessionId || msg.characterId
        if (!lastMsgMap[key] || (msg.timestamp || 0) > (lastMsgMap[key].timestamp || 0)) lastMsgMap[key] = msg
      }
      chatList.value = chats.map((chat) => {
        const profile = profiles.find((item) => String(item.id) === String(chat.profileId)) || {}
        return { ...chat, profile, lastMsg: lastMsgMap[chat.sessionId || chat.characterId] || null }
      }).reverse()
      return
    }
