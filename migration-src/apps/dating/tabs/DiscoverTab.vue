<template>
  <div class="tab-discover">
    ...
    <!-- 主滚动区域 -->
    <div class="scroll-container" @scroll.passive="onFeedScroll">
      <!-- 推荐流顶部功能入口 -->
...
const onFeedScroll = async (event) => {
  if (event.currentTarget.scrollTop <= -60) await forceRefresh()
}
  ...
  try {
    if (targetObj.realCharId) {
      ...
      if (realChar) {
        ...
        const profile = bridge.hasSdk() ? await bridge.dbCreate('profiles', fakeProfile) : { id: await db.dating_profiles.add(fakeProfile) }
        const match = bridge.hasSdk() ? await bridge.dbCreate('matches', {
          profileId: profile.id,
          characterId: targetObj.realCharId,
          messageCount: 0,
          status: 'active',
          type: 'feed'
        }) : { id: await db.dating_chats.add({ profileId: profile.id, messageCount: 0, status: 'active', type: 'feed' }) }

        window.dispatchEvent(new CustomEvent('dating-refresh-chats'))
        emit('open-chat', match.id)
        return
      }
    }
