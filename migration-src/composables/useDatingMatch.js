...
要求：必须返回纯 JSON 格式数据。你必须严格遵守以下 JSON 结构的所有字段，不得删减任何属性：
{
  ...
  "NSFW_info": {
    ...
    "limits": ["禁忌/底线"]
      ...
      if (bridge.hasSdk()) {
        const profile = await bridge.dbCreate('profiles', {
        ...
        const match = await bridge.dbCreate('matches', {
          profileId: profile.id,
          characterId: profile.realCharId || null,
        ...
        isGenerating.value = false
        return { success: true, profileId: profile.id, chatId: match.id }
      }
