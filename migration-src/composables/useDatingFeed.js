...
export function useDatingFeed() {
  ...
  const persistPost = async (post, feed = null) => {
    if (!bridge.hasSdk() || !post?.id) return
    const collectionPost = { ...toPlainPost(post), feed: feed || post.feed || 'recommend' }
    const existing = await bridge.dbGet('posts', post.id)
    if (existing) await bridge.dbUpdate('posts', post.id, collectionPost)
    else await bridge.dbCreate('posts', collectionPost)
  }

  const hydrateSdkData = async () => {
    if (!bridge.hasSdk() || sdkDataHydrated) return
    sdkDataHydrated = true
    try {
      const [storedPosts, storedNotifications] = await Promise.all([
        bridge.dbList('posts'),
        bridge.listNotifications()
      ])
      if (storedPosts.length) {
        recommendPosts.value = storedPosts.filter((p) => (p.feed || 'recommend') === 'recommend').map(normalizePost)
        followingPosts.value = storedPosts.filter((p) => p.feed === 'following').map(normalizePost)
        myPosts.value = storedPosts.filter((p) => p.feed === 'mine').map(normalizePost)
      }
      if (storedNotifications.length) notifications.value = storedNotifications.map(normalizeNotification)
    } catch (error) {
      console.warn('coldpark SDK 数据读取失败，继续使用本地缓存', error)
    }
  }
  ...
  const addNotification = async (payload) => {
    ...
    if (bridge.hasSdk()) await bridge.createNotification(item)
    return item
  }
...
[
  ...
  const generateFollowing = async () => {
    try {
      const activeChats = bridge.hasSdk()
        ? (await bridge.dbList('matches')).filter((chat) => chat.status !== 'exited')
        : await db.dating_chats.where('status').notEqual('exited').toArray()
      ...
      for (const pid of profileIds) {
        const p = bridge.hasSdk() ? await bridge.dbGet('profiles', pid) : await db.dating_profiles.get(pid)
...
[
  ...
  const postComment = async (post, text, options = {}) => {
    ...
    post.comments = sanitizeComments(post.comments)
    void persistPost(post, post.isMine ? 'mine' : 'recommend')
  ...
  const markAllRead = async () => {
    ...
    if (bridge.hasSdk()) await bridge.markAllNotificationsRead()
  }

  const markNotificationRead = async (id) => {
    const target = notifications.value.find(n => n.id === id)
    if (target) target.isRead = true
    if (bridge.hasSdk() && target) await bridge.dbUpdate('notifications', id, { isRead: true })
  }
