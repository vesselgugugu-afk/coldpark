...
export function useAiPhoneBridge() {
  ...
  const chat = async (options = {}) => {
    ...
    var finalOptions = isArrayOptions ? messages : Object.assign({}, options, { messages: messages });

    if (!hasSdk()) {
      var last = messages[messages.length - 1];
      return { text: '[coldpark demo] ' + (last && last.content ? String(last.content).slice(0, 80) : 'AI reply') };
    }
